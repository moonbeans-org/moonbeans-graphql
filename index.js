...
const cn = 'postgres://postgres:<DBPASS>@<DBHOST>:5432/moonbeans';
const db = pgp(cn);
...
let marketPlaceAddress = "0x16d7Edd3A562BB60aA0B3Af357A2c195cE2AA974";

const web3 = new Web3(new Web3.providers.WebsocketProvider("wss://service.elara.patract.io/Moonriver/01b02adf529587dc262205122d04a59a", {
    clientConfig: {
...
async function startListening() {
    startBlock = parseInt(await fs.readFile("last_block.txt"));
...
async function handleLogs() {
    try {
        while (true) {
            ...
            let events = await contract.getPastEvents("allEvents", { 'fromBlock': startBlock, 'toBlock': endBlock });
...
async function handleTokenDelisted(row) {
    ...
    let collection = await db.oneOrNone('SELECT * FROM "collections" WHERE "id" = $1', [row['returnValues']['token']]);
    if (collection === null) {
        throw "Collection not in database";
    }
...
async function handleTokenPurchased(row) {
    ...
    if (methodSignatures[tx.input.slice(2, 10)] == "acceptOffer") {
        ...
        // REMOVE CURRENT BID
        let bid = await db.oneOrNone('SELECT * FROM "bids" WHERE "tokenId" = $1 AND "buyer" = $2', [id, row['returnValues']['newOwner']]);
        if (bid !== null) {
            // SAVE FILL
