...
const cn = 'postgres://janakmalla:postgres@localhost/moonbeanstwochainz';
const db = pgp(cn);
const ABIS = require("./utils/abis.js");
const CHAIN_NAME = "arbitrum_goerli";
...
const web3 = new Web3(new Web3.providers.WebsocketProvider("wss://arb-goerli.g.alchemy.com/v2/jFrGjdbYIJS341_qYgB37b_ntcCw_ehq", {
    clientConfig: {
...
async function startListeningMarketplace() {
    let startBlock = parseInt(await fs.readFile("last_block_arbi_goerli.txt"));
...
async function startListeningHolders() {
    ...
    let collections = JSON.parse(await fs.readFile('./utils/test_collections.json'));
